...
本项目使用 GitHub Actions 实现自动编译和部署。

### 触发条件

| 环境 | 分支 | 触发条件 |
|------|------|----------|
| 测试环境 | `test` | 向 test 分支推送代码 |
| 生产环境 | `prov` | 向 prov 分支推送代码 |

### 编译流程

1. **环境准备**：使用 Python 3.9 运行环境
2. **依赖安装**：根据 `requirements.txt` 安装依赖
3. **PyInstaller 编译**：将项目打包为 Linux 可执行文件 `weather`
4. **资源打包**：自动打包 `memory/` 和 `model/` 目录下的文件
5. **部署**：通过 SSH 将编译结果发送到目标服务器

### 部署目标

| 环境 | 服务器 | 部署路径 |
|------|--------|----------|
| 测试环境 | test.stoprefactoring.com:22 | `/public/engine/` |
| 生产环境 | prov.stoprefactoring.com:22 | `/public/engine/` |
...
| Secret 名称 | 说明 |
|-------------|------|
| `TEST_SSH_KEY` | 测试环境 SSH 私钥 |
| `PROV_SSH_KEY` | 生产环境 SSH 私钥 |
